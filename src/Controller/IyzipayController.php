...
class IyzipayController extends ControllerBase {
  ...
  public function complete($cart_id = 0, Request $request) {
    ...
    $include_yolu = $_SERVER['DOCUMENT_ROOT'].'/modules/ubercart/payment/uc_iyzipay/iyzipay/src/Iyzipay/samples/config.php';
		require_once($include_yolu);

		$request = new \Iyzipay\Request\RetrieveCheckoutFormRequest();
    $request->setToken($token);
    drupal_set_message("token=".$mytoken);
    ...
    drupal_set_message($checkoutForm->getStatus());
    drupal_set_message($checkoutForm->getPaymentStatus());
    drupal_set_message($checkoutForm->getErrorMessage());

    //drupal_set_message($request->getToken());
    \Drupal::logger('uc_iyzipay')->notice('Receiving new order notification for order @order_id.', ['@order_id' => SafeMarkup::checkPlain($request->request->get('merchant_order_id'))]);
    ...
    if (Unicode::strtolower($key) != Unicode::strtolower($valid)) {
      uc_order_comment_save($order->id(), 0, $this->t('Attempted unverified Iyzipay completion for this order.'), 'admin');
      throw new AccessDeniedHttpException();
    }

    if ($request->request->get('demo') == 'Y' xor $configuration['demo']) {
      \Drupal::logger('uc_iyzipay')->error('The Iyzipay payment for order <a href=":order_url">@order_id</a> demo flag was set to %flag, but the module is set to %mode mode.', array(
    ...
    if ($request->request->get('credit_card_processes') == 'Y' && is_numeric($request->request->get('total'))) {
      $comment = $this->t('Paid by @type, Iyzipay.com order #@order.', ['@type' => $request->request->get('pay_method') == 'CC' ? $this->t('credit card') : $this->t('echeck'), '@order' => SafeMarkup::checkPlain($request->request->get('order_number'))]);
      uc_payment_enter($order->id(), 'Iyzipay', $request->request->get('total'), 0, NULL, $comment);
    }
    else {
      drupal_set_message($this->t('Your order will be processed as soon as your payment clears at Iyzipay.com.'));
      uc_order_comment_save($order->id(), 0, $this->t('@type payment is pending approval at Iyzico.com.', ['@type' => $request->request->get('pay_method') == 'CC' ? $this->t('Credit card') : $this->t('eCheck')]), 'admin');
    }
